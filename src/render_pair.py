...
# panel 1: GDP world
grid, X, Y, p = frame_mesh(gdp_exp); m = json.load(open(os.path.join(ROOT, "experiments", gdp_exp, "metrics.json")))
tot = m.get("population", m.get("total", 0))
...
grid2, X2, Y2, p2 = frame_mesh(py_exp); m2 = json.load(open(os.path.join(ROOT, "experiments", py_exp, "metrics.json"))); tot2 = m2.get("population", m2.get("total", 0))
...
Gs = ndimage.gaussian_filter(G, 3); Ps = ndimage.gaussian_filter(PY, 3)
...
draw_hero(X2, Y2, p2, os.path.join(out, "gdp_per_personyear.png"), out_w, title="GDP PER YEAR OF HUMAN LIFE", legend_text="= 10 billion person-years", legend_unit=1e10,
          ...
          source="Kummu GDP PPP 2015 divided by HYDE person-years since 10,000 BC, per 5 arcmin cell, smoothed 3 cells; log2 scale, RdBu.",

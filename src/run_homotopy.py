"""Spectral homotopy at one resolution, every stage saved as its own experiment (metrics + renders).
    python src/run_homotopy.py <prefix> <width> <sigma_km> <shares> [iters] [ocean_share] [lon0] [x_boundary]"""
...
po, stages = ot_poisson.spectral_homotopy(P, shares, sigma_px, xb, iters=iters, damping=0.5, log=lambda s: None, on_stage=on_stage, ocean=ocean, ocean_share=ocean_share)

...
for e in exps:
    ...
    panels.append((png, f"{-y} BC" if y < 0 else f"{y} AD", f"{pop/1e9:.2f} billion" if pop >= 1e9 else f"{pop/1e6:.0f} million", p.get("honesty", "")))
cols = 4; rows = (len(panels) + cols - 1) // cols
im0 = Image.open(panels[0][0]); w, h = im0.size; cap_h = int(0.09 * h); gap = int(0.02 * w); title_h = int(0.10 * h)
sheet = Image.new("RGB", (cols * w + (cols + 1) * gap, title_h + rows * (h + cap_h + gap) + gap), "white")
dr = ImageDraw.Draw(sheet)
try:
    fb = ImageFont.truetype("/System/Library/Fonts/HelveticaNeue.ttc", int(0.055 * h)); fs = ImageFont.truetype("/System/Library/Fonts/HelveticaNeue.ttc", int(0.032 * h)); ft = ImageFont.truetype("/System/Library/Fonts/HelveticaNeue.ttc", int(0.06 * h))
except Exception:
    fb = fs = ft = ImageFont.load_default()
dr.text((gap, int(0.25 * title_h)), "THE SAME RECTANGLE, TWELVE THOUSAND YEARS: AREA = PEOPLE", font=ft, fill="#111")
dr.text((gap, int(0.65 * title_h)), "Each panel is the world with area proportional to the people alive that year; the frame is always full, so the caption says how many people it holds. HYDE 3.3 to 2023, SSP2 projection after.", font=fs, fill="#444")
for i, (png, yr, pop, hon) in enumerate(panels):
    ...
    dr.text((x0, y0 + h + int(0.15 * cap_h)), yr, font=fb, fill="#111"); dr.text((x0 + int(0.32 * w), y0 + h + int(0.22 * cap_h)), f"{pop} · {hon}", font=fs, fill="#444")

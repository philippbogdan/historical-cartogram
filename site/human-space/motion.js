...
export function advanceMotion(position,target,seconds,pressure,elapsed=0) {
  const steps=Math.max(1,Math.ceil(seconds*240)),dt=seconds/steps;
  for(let i=0;i<steps;i++) {
    const launch=1-Math.exp(-(elapsed+(i+.5)*dt)/.18);
    const rate=launch*(0.7+1.6*Math.sqrt(Math.max(0,pressureAt(position,pressure))));

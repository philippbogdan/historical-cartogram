...
export function createPaperPolygons(canvas,warp,n,image,data,sourceVertices,pull){
...
void main(){
  ...
  vec2 position=mix(geography,target,progress);
...
void main(){
  if(source.y>${southLimit})discard;
  ...
  ['warp','atlas','colours','pullField'].forEach((name,i)=>gl.uniform1i(gl.getUniformLocation(program,name),i));
  ...
  return ({width,height,dpr,fit,zoom,cx,cy,progress,colourMode,gravity})=>{
    ...
    gl.bindVertexArray(edgeVao);gl.uniform1i(lines,1);gl.drawArrays(gl.LINES,0,edgeCursor/2);
  };
}

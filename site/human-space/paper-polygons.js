...
export function createPaperPolygons(canvas,warp,n,image,data,sourceVertices){
  const edges=new Float32Array(sourceVertices.length*2);let edgeCursor=0;
  for(let i=0;i<data.sites.length;i++){
    const a=data.offsets[i],b=data.offsets[i+1];
    for(let j=a;j<b;j++){const next=j+1===b?a:j+1;edges.set([sourceVertices[j*2],sourceVertices[j*2+1],sourceVertices[next*2],sourceVertices[next*2+1]],edgeCursor);edgeCursor+=4;}
  }
...
void main(){
  ...
  vec2 position=mix(uv,target,progress);
